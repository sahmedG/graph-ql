...
export async function getTitleData(
  userId: number
): Promise<{ firstName: string; level: number }> {
  const query = `
...
export async function getAuditData(
  userId: number
): Promise<{ auditRatio: number; totalUp: number; totalDown: number }> {
  const query = `
...
export async function getLatestFinishedAudit(
  userId: number
): Promise<{ projectName: string; captain: string; didPass: boolean }> {
  const query = `
...
export async function getLatestFinishedProject(
  userId: number
): Promise<string> {
  const query = `
...
export async function getXpForProjects(
  userId: number
  ...
  return new Promise((resolve, reject) => {
    fetch(`${baseUrl}/api/graphql-engine/v1/graphql`, {
      ...
      .then((data) => {
        ...
        resolve(
          data.data.transaction.map((t: { amount: number; path: string }) => ({
            amount: t.amount,
...
export async function getAuditsWithEvents(userId: number): Promise<Audit[]> {
  // Fetch audits
  ...
  const audits: Audit[] = auditData.data.audit;
  ...
  const auditsWithEvents: Audit[] = audits.map((audit) => ({
    ...audit,
...
function MyTest(
  paths: { amount: number; path: string }[],
  key: string
): number {
...
export async function getWorkingOn(userId: number) {
  const workingOnData = await fetch(
    `${baseUrl}/api/graphql-engine/v1/graphql`,
    {
      method: "POST",
      headers: {
  ...
  if (!workingOnData.ok) {
    throw new Error(
      `Could not fetch audits: ${workingOnData.status} ${workingOnData.statusText}`
    );
  }
  ...
  if (currentProject.errors) {
    throw new Error(
      `Could not fetch audits: ${currentProject.errors[0].message}`
    );
  }
