import { getAuditData, getLatestFinishedAudit, getLatestFinishedProject, getTitleData, getXpForProjects, getAuditsWithEvents, getSkills, getWorkingOn, } from "./network.js";
...
export default function homeHandler() {
    ...
    ]), renderTitle(), renderStats(), renderBottom());
}
...
function makeLine(length, color) {
    ...
    getXpForProjects(parseJwt().userId)
        ...
        projects.forEach((project) => {
            ...
            pieData.push({
                ...
                color: color,
                label: `${project.name} - ${((project.amount / totalAmount) *
                    100).toFixed(1)}%`,
            });
        });
...
function bottomTable() {
    ...
    const renderTable = (filteredAudits) => {
        ...
        filteredAudits.forEach((audit) => {
            ...
            else if ((audit.grade !== null && audit.grade < 1) ||
                audit.grade === null) {
                value_xp = "0";
            }
            else {
    ...
    const filterAudits = (status) => {
        if (status === "all") {
            ...
            renderTable(auditsData.filter((audit) => audit.grade !== null && audit.grade < 1));
        }
        else {
            renderTable(auditsData.filter((audit) => audit.grade !== null && audit.grade >= 1));
        }
...
function workingOn() {
    ...
    getWorkingOn(parseJwt().userId).then((data) => {
        ...
        startedOnDate.innerHTML = `${new Date(data.data.workingGroup[0].updatedAt).toLocaleDateString()}`;
    });
    return container;
}
